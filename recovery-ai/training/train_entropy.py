...
def load_dataset(path: Path) -> tuple[list[str], list[int]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line in handle:
            ...
            labels.append(int(record["labels"]["entropy"]["value"]))
    return texts, labels
...
def main() -> None:
    ...
    model.save(output)
    metrics = {"model": "entropy", "train_rows": len(texts), "positive_rate": round(sum(labels) / max(1, len(labels)), 6)}
    (config.model_dir / "entropy.metrics.json").write_text(json.dumps(metrics, indent=2), encoding="utf-8")
